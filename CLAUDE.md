...
# S3 sync commands
npm run sync:sources-to-s3        # Upload local source files to S3 (srefs/ prefix)
...
- **S3 Sync Pipeline**: 
  ...
  3. Process images with Astro
  4. Upload optimized images to S3 CDN (`sync:to-s3`)
...
### Architecture
- **Original Images**: Stored in S3 under `srefs/sref-{id}/images/` and `srefs/sref-{id}/meta.yaml`
- **Processed Images**: Generated by Astro and stored in S3 under `cdn/processed/`
- **CDN Delivery**: CloudFront serves all processed images with proper caching headers
- **Build Pipeline**: 
  1. `sync:sources-to-s3` - Upload any new local source files to S3
  2. `sync:from-s3` - Download all source files from S3 to local
  3. `astro build` - Process images and generate static site
  4. `sync:to-s3` - Upload optimized images to S3 CDN
